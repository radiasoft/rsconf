...
class T(component.T):
    ...
    def _check_min_version(self, z):
        z.setdefault("min_version", "0.0.0")
        p = [int(v) for v in z.min_version.split(".")]
        if len(p) != 3:
            raise ValueError(f"min_version={z.min_version} needs to be in X.X.X format")
        z.min_version = p
    ...
    def _setup_tls_host(self, j2_ctx, z):
        ...
        assert ipaddress.ip_address(
            pkcompat.locale_str(z.tls.ip)
        ).is_private, "tls_host={} is on public ip={}".format(z.tls_host, z.tls.ip)
